...
def search():
    try:
        ...
        ck = _cache_key('search', gene_name, min_confidence, max_confidence, column, sort_order)
        cached = cache.get(ck)
        if cached:
            cached['cached'] = True
...
def get_gene_suggestions():
    try:
        gene_file = Path("/home/user-kp/Documents/GitHub/SLxGO_website/name_files/gene_names.txt")
